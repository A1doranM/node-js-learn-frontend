...
class Feed extends Component {
    ...
    loadPosts = direction => {
        ...
        fetch('http://localhost:8080/graphql', {
            ...
            .then(resData => {
                if (resData.errors) {
                    throw new Error('Fetching posts failed: ' + resData.errors[0].message);
                }
                this.setState({
    ...
    statusUpdateHandler = event => {
        ...
        fetch('http://localhost:8080/graphql', {
            ...
            .then(resData => {
                if (resData.errors) {
                    throw new Error('Update status failed: ' + resData.errors[0].message);
                }
                this.setState({
    ...
    deletePostHandler = postId => {
        ...
        fetch(`http://localhost:8080/graphql`, {
            ...
            .then(resData => {
                if (resData.errors) {
                    throw new Error('Deleting failed: ' + resData.errors[0].message);
                }
                this.loadPosts();
                // this.setState(prevState => {
